...
public class DependencyDecoder2O extends DependencyDecoder {
	...
	while(true) {
		...
		for(int pa = 0; pa < par.length; pa++) {
			...
			max = change+change1; wh = ch; nPar = pa; nType = (pipe.labeled && !pipe.separateLab) ? static_types[pa][ch] : 0;
		    }
		}
	    }
	    if(max <= 0.0)
	...
	int[] headsGold = new int[inst.heads.length];
	String[] labelsGold = new String[inst.deprels.length];			
