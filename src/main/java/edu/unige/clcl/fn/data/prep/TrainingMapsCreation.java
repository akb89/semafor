...
public class TrainingMapsCreation {
	...
	public static void main(String[] args) throws IOException {
		...
		THashMap<String,THashSet<String>> newFEMap =
				tMapCreation.createFrameFEMap(feSplitsFile);
		THashMap<String,THashSet<String>> oldFEMap =
				(THashMap<String,THashSet<String>>) SerializedObjects
				.readSerializedObject("/Users/AKB/Dropbox/GitHub/semafor/data/framenet.frame.element.map.old");
		THashMap<String,THashSet<String>> oldLUMap =
				(THashMap<String,THashSet<String>>)SerializedObjects
				.readSerializedObject("/Users/AKB/Dropbox/GitHub/semafor/data/framenet.original.map.old");
		THashMap<String,THashSet<String>> newLUMap =
				tMapCreation.createFrameLUMap(feSplitsFile, posTaggedSplitsFile);
		tMapCreation.logger.info("Comparing keys (frames) between old and new Frame Element Maps");
		tMapCreation.compareKeys(oldFEMap, newFEMap);
		tMapCreation.logger.info("Comparing keys (frames) between old and new Lexical Unit Maps");
		tMapCreation.compareKeys(oldLUMap, newLUMap);
		tMapCreation.logger.info("Comparing values (FEs) between old and new Frame Element Maps");
		tMapCreation.compareValues(oldFEMap, newFEMap);
		tMapCreation.logger.info("Comparing values (LUs) between old and new Lexical Unit Maps");
	 ...
	 * @author dipanjan
	 */
	private void compareKeys(THashMap<String,THashSet<String>> originalMap, THashMap<String,THashSet<String>> newMap){
		...
		for(Map.Entry<String, THashSet<String>> entry : newMap.entrySet()){
			if(!originalMap.containsKey(entry.getKey())){
				logger.info("Original map does not contain frame: " + entry.getKey());
				counter+=1;
			}
		}
		if(counter != 0){
		...
		for(Map.Entry<String, THashSet<String>> entry : originalMap.entrySet()){
			if(!newMap.containsKey(entry.getKey())){
				logger.info("New map does not contain frame: "+entry.getKey());
				counter += 1;
			}
		}
	...
	private void compareValues(THashMap<String,THashSet<String>> originalMap, THashMap<String,THashSet<String>> newMap){
		int counter = 0;
		for(Map.Entry<String, THashSet<String>> entry : newMap.entrySet()){
			if(originalMap.containsKey(entry.getKey())){
				THashSet<String> originalValues = originalMap.get(entry.getKey());
				THashSet<String> newValues = entry.getValue();
				if(originalValues.size() != newValues.size()){
					logger.info("Different values set size for frame: " + entry.getKey());
					logger.info("	Original values set size = " + originalValues.size());
					logger.info("	New values set size = " + newValues.size());
					for(String value : newValues){
						if(!originalValues.contains(value)){
							logger.info("		Value = " + value + " is not in the original map");
							counter += 1;
						}
					}
					if(counter != 0){
					...
					for(String value : originalValues){
						if(!newValues.contains(value)){
							logger.info("		Value = " + value + " is not in the new map");
							counter += 1;
						}
					}
				}
	...
	private THashMap<String,THashSet<String>> createFrameFEMap(String frameElementSplits)
			throws IOException {
	...
	private List<Integer> getTargetIndexes(List<String> feSplitTokens){
		return splitBy(feSplitTokens
				.get(FE_SPLITS_TARGET_INDEX), "_")
	...
	private String getTaggedLU(List<String> taggedTokens, List<Integer> targetIndexes){
		String taggedLU = "";
	...
	private THashMap<String,THashSet<String>> createFrameLUMap(
		...
		List<String> posTaggedSentences = Files.lines(Paths.get(posTaggedSplits))
		...
		Files.lines(Paths.get(frameElementSplits)).forEach((line) -> {
			...
			List<String> taggedTokens = splitByWhiteSpace(posTaggedSentences.get(sentenceIndex));
