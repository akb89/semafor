...
public class ScoringRequiredDataCreation {
	private static final Logger logger = LoggerFactory.getLogger(
			ScoringRequiredDataCreation.class);
	...
	public static void main(String[] args) throws IOException {
		...
		final String outputFrRelationFile =
				preprocessedDataDir + "/frRelations.xml";
		scoringReqDataC.createFramesXmlFile(frameDir, outputFramesFile);
		scoringReqDataC.createFrRelationsXmlFile(frRelationFile,
												 outputFrRelationFile);
		logger.info("Done creating frames.xml and feRelations.xml files");
	}

	private Element createFrameImportElement(Document frameDoc,
											 Document frameSingleDoc) {
		...
		for (int i = 0; i < frameChildren.getLength(); i++) {
			...
			case "FE":
				...
				Node feNode = frameSingleDoc.importNode(frameChildren.item(i),
														false);
				...
				for (int j = 0; j < feChildren.getLength(); j++) {
					...
					if (feChildNodeName.equals("definition")) {
						feNode.appendChild(frameSingleDoc.importNode(
								feChildren.item(j), true));
					}
					// Append the semTypes
					if (feChildNodeName.equals("semType")) {
						semTypes.appendChild(frameSingleDoc.importNode(
								feChildren.item(j), true));
					}
					// Append excludesFE
	...
	private void createFramesXmlFile(String frameDir, String outputFile)
			throws IOException {
		...
		framesRootElement.setAttribute("XMLCreated",
									   dateFormat.format(dateObj));
		Files.walk(Paths.get(frameDir)).forEach(filePath -> {
			if (Files.isRegularFile(filePath) && filePath.getFileName()
														 .toString().endsWith(
							".xml")) {
				Document frameDoc = XmlUtils.parseXmlFile(filePath.toString(),
														  false);
				if (frameDoc != null) {
					Element frameImportElement = createFrameImportElement(
							frameDoc, frameSingleDoc);
	...
	private void createFrRelationsXmlFile(String frRelationFile,
										  String outputFile) {
		Document frRelationDoc = XmlUtils.parseXmlFile(frRelationFile, false);
		NodeList frRelationTypeNodeList = frRelationDoc.getElementsByTagName(
				"frameRelationType");
		for (int i = 0; i < frRelationTypeNodeList.getLength(); i++) {
			Element frameRelationsElement = frRelationDoc.createElement(
					"frame-relations");
			while (frRelationTypeNodeList.item(i).hasChildNodes()) {
				Node firstChild = frRelationTypeNodeList.item(i)
														.getFirstChild();
				if (firstChild.getNodeName().equals("frameRelation")) {
					frameRelationsElement.appendChild(firstChild);
				}
				frRelationTypeNodeList.item(i).removeChild(
						frRelationTypeNodeList.item(i).getFirstChild());
			}
			frRelationTypeNodeList.item(i).appendChild(frameRelationsElement);
		}
		// Rename frameRelations --> fr-relations
		NodeList frameRelationsNodes = frRelationDoc.getElementsByTagName(
				"frameRelations");
		for (int i = 0; i < frameRelationsNodes.getLength(); i++) {
			frRelationDoc.renameNode(frameRelationsNodes.item(i), null,
									 "fr-relations");
		}
		// Rename frameRelationType --> frame-relation-type
		NodeList frameRelationsTypeNodes = frRelationDoc.getElementsByTagName(
				"frameRelationType");
		for (int i = 0; i < frameRelationsTypeNodes.getLength(); i++) {
			frRelationDoc.renameNode(frameRelationsTypeNodes.item(i), null,
									 "frame-relation-type");
		}
		// Rename frameRelation --> frame-relation
		NodeList frameRelationNodes = frRelationDoc.getElementsByTagName(
				"frameRelation");
		for (int i = 0; i < frameRelationNodes.getLength(); i++) {
			frRelationDoc.renameNode(frameRelationNodes.item(i), null,
									 "frame-relation");
		}
		// Rename FERelation --> fe-relation
		NodeList feRelatioNodes = frRelationDoc.getElementsByTagName(
				"FERelation");
		for (int i = 0; i < feRelatioNodes.getLength(); i++) {
			frRelationDoc.renameNode(feRelatioNodes.item(i), null,
									 "fe-relation");
		}
