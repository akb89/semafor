...
public class LatentAlphabetCreationThreaded {
	private static final Logger logger = LoggerFactory
			.getLogger(LatentAlphabetCreationThreaded.class);

	private static final int BATCH_SIZE = 100;
	private static int EXPECTED_NUM_FEATURES = 3000000;
	public static final String ALPHABET_FILENAME = "alphabet.dat";
	final static FilenameFilter LOCAL_ALPHABET_FILENAME_FILTER = (dir, filename) -> filename
			.startsWith(ALPHABET_FILENAME + "_");
	...
	public static void main(String[] args)
		...
		final RequiredDataForFrameIdentification r = SerializedObjects
				.readObject(options.fnIdReqDataFile.get());
		final Relations wnRelations = new CachedRelations(r.getRevisedRelMap(),
		...
		final LatentAlphabetCreationThreaded events = new LatentAlphabetCreationThreaded(
				alphabetDir, options.trainFrameElementFile.get(),
				options.trainParseFile.get(), r.getFrameMap(),
				new LatentFeatureExtractor(wnRelations, lemmatizer), startIndex,
				endIndex, numThreads);
	...
	public LatentAlphabetCreationThreaded(File alphabetDir,
			String frameElementsFile, String parseFile,
			THashMap<String, THashSet<String>> frameMap,
			LatentFeatureExtractor featureExtractor, int startIndex,
	...
	public void createLocalAlphabets() throws IOException {
		final List<String> frameLines = Files
				.readLines(new File(frameElementsFile), Charsets.UTF_8)
				.subList(startIndex, endIndex);
		final List<String> parseLines = Files
				.readLines(new File(parseFile), Charsets.UTF_8);
	...
	private void processBatch(int threadId, List<String> frameLines,
	...
	private void processLine(String frameLine, List<String> parseLines,
			Set<String> alphabet) {
		// Parse the frameLine
		...
		for (String frame : frameMap.keySet()) {
			extractFeaturesAndAddToAlphabet(frame, targetTokenIdxs, data,
					alphabet);
		}
	}

	private void extractFeaturesAndAddToAlphabet(String frame,
			int[] targetTokenIdxs, String[][] allLemmaTags,
			Set<String> alphabet) {
		final THashSet<String> hiddenUnits = frameMap.get(frame);
		final DependencyParse parse = DependencyParse
				.processFN(allLemmaTags, 0.0);
		for (String unit : hiddenUnits) {
			final IntCounter<String> valMap = featureExtractor
					.extractFeatures(frame, targetTokenIdxs, unit, allLemmaTags,
							parse, true);
	...
	public static void combineAlphabets(File alphabetDir) throws IOException {
		...
		writeAlphabetFile(alphabet,
				alphabetDir.getAbsolutePath() + "/" + ALPHABET_FILENAME);
	}

	public static Map<String, Integer> readAlphabetFile(String filename)
			throws IOException {
	...
	public static void writeAlphabetFile(Set<String> alphabet, String filename)
			throws IOException {
