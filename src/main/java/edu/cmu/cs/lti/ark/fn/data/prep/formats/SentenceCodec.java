...
public abstract class SentenceCodec {
	...
	public static final SentenceCodec ConllCodec = new SentenceCodec("\n\n",
			"\n") {
		@Override public Token decodeToken(String tokenStr) {
			return Token.fromConll(tokenStr);
		}

		@Override public String encodeToken(Token token) {
	...
	public static final SentenceCodec PosTaggedCodec = new SentenceCodec("\n",
			" ") {
		@Override public Token decodeToken(String tokenStr) {
			return Token.fromPosTagged(tokenStr);
		}

		@Override public String encodeToken(Token token) {
			return token.toPosTagged();
		}
	};
	...
	public static final SentenceCodec MaltCodec = new SentenceCodec("\t\\d*\n",
			"\t1\n", " ") {
		@Override public Token decodeToken(String tokenStr) {
			return Token.fromMalt(tokenStr);
		}
	...
	protected SentenceCodec(String sentenceDelimiterPattern,
			String sentenceDelimiter, String tokenDelimiter) {
	 ...
	 */
	public static class SentenceIterator extends AbstractIterator<Sentence>
		...
		public SentenceIterator(SentenceCodec codec, Readable input) {
			...
			this.scanner = new Scanner(input)
					.useDelimiter(codec.sentenceDelimiter);
		}

		public SentenceIterator(SentenceCodec codec, File file)
				throws FileNotFoundException {
			this(codec, new FileReader(file));
		}

		@Override protected Sentence computeNext() {
			if (scanner.hasNext())
	...
	public static class SentenceWriter implements Closeable {
		...
		public SentenceWriter(SentenceCodec codec, File file)
				throws IOException {
