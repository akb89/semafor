...
public class Semafor {
	...
	private static final String FRAME_ELEMENT_MAP_FILENAME = "data/framenet.frame.element.map";
	...
	public void runParser(final InputSupplier<? extends Readable> inputSupplier,
		...
		try {
			...
			final Thread consumer = new Thread(new Runnable() {
				@Override public void run() {
					while (!Thread.currentThread().isInterrupted()) {
						try {
							final Optional<SemaforParseResult> oResult = results
									.take().get();
			...
			final SentenceCodec.SentenceIterator sentences = ConllCodec
					.readInput(inputSupplier.getInput());
			try {
				int i = 0;
	...
	public SemaforParseResult parseSentence(Sentence unLemmatizedSentence)
			throws IOException {
		// look up lemmas
		...
		final List<Pair<List<Integer>, String>> idResult = predictFrames(
				sentence, segments);
	...
	public List<String> getArgumentIdInput(Sentence sentence,
		...
		for (Pair<List<Integer>, String> targetAndFrame : idResults) {
			...
			final Pair<String, String> tokenRepresentation = getTokenRepresentation(
